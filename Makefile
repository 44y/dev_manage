...
### The docker/$(PKG_NAME) is main docker dir
BIN_DIR    := $(BUILD_ROOT)/docker/$(PKG_NAME)/output
OUT_PUT    := $(BIN_DIR)
...
test:
	$(MAKE) -C docker/$(PKG_NAME)  test

tools:.toolchain
	@:

.toolchain:toolchain/Dockerfile
	docker build --rm  -f toolchain/Dockerfile -t builder/alpine/golang-$(PKG_NAME)  toolchain
	@[ $$? -eq 0 ] && touch $@
...
help:
	...
	@printf "clean-deploy \t\t- clean deploy for make deploy\n"
